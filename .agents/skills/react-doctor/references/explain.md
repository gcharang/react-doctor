...
```bash
npx github:gcharang/react-doctor#pinned rules explain react-doctor/no-array-index-as-key
```
...
```bash
npx github:gcharang/react-doctor#pinned rules list                         # every rule + its effective severity
npx github:gcharang/react-doctor#pinned rules list --configured            # only what your config changed
npx github:gcharang/react-doctor#pinned rules list --category Performance   # filter by category
npx github:gcharang/react-doctor#pinned rules explain <rule>               # why it matters + how to configure
npx github:gcharang/react-doctor#pinned rules disable <rule>               # rule never runs
npx github:gcharang/react-doctor#pinned rules enable <rule>                # turn back on at its recommended severity
npx github:gcharang/react-doctor#pinned rules set <rule> warn              # off | warn | error
npx github:gcharang/react-doctor#pinned rules category "React Native" off   # whole category
npx github:gcharang/react-doctor#pinned rules ignore-tag design            # skip a rule family (design, test-noise, …)
npx github:gcharang/react-doctor#pinned rules unignore-tag design
```
