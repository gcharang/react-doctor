...
describe("render-github-action-comment", () => {
  ...
  it("renders a sticky markdown summary and action outputs from a JSON report", () => {
    ...
    expect(comment).toContain(
      "<sub>Reviewed by [React Doctor](https://github.com/gcharang/react-doctor) for commit `cfc8878`. See inline comments for fixes.</sub>",
    );
  ...
  it("renders a clean baseline report with no new issues", () => {
    ...
    expect(comment).toContain(
      "<sub>Reviewed by [React Doctor](https://github.com/gcharang/react-doctor) for commit `cfc8878`.</sub>",
    );
  ...
  it("renders a no-scan report as a plain success line, not a zero-filled table", () => {
    ...
    expect(comment).toContain("<sub>Reviewed by [React Doctor](https://github.com/gcharang/react-doctor) for commit");
