...
import { DIFF_PARENT_BASE } from "../utils/resolve-scope.js";
...
export const inspectAction = async (directory: string, flags: InspectFlags): Promise<void> => {
  ...
  try {
    ...
    // `parent` is a reserved base sentinel (auto-detect the forked-from
    // branch via nearest merge-base), not a literal base ref — route it to
    // parent detection and keep the explicit base unset.
    const wantsParentBranch = scopeRequest.base === DIFF_PARENT_BASE;
    ...
    const diffInfo =
      ...
      (shouldDetectDiff
        ? await getDiffInfo(resolvedDirectory, wantsParentBranch ? undefined : scopeRequest.base, {
