...
const buildDependencyFollowUp = (
  ...
  return `  React Doctor still works via \`npx ${FORK_PACKAGE_SPEC}\`. To install locally: ${installCommand}`;
};

// Adds the `doctor` (or `react-doctor`) script to package.json so users can
// run `pnpm doctor` / `npm run doctor`. The script invokes `npx ${FORK_PACKAGE_SPEC}`,
...
export const installReactDoctorPackageSetup = async (
  ...
  try {
    ...
    if (dependencyResult.dependencyStatus === "skipped") {
      ...
      // dep, but `npx ${FORK_PACKAGE_SPEC}` still works. Use spinner.warn (⚠) so it
