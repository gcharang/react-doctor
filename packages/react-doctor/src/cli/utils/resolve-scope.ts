...
/**
 * Reserved base value that triggers parent-branch auto-detection — diff
 * against the branch HEAD forked from (nearest merge-base) instead of the
 * repo default branch. Stays a plain string through scope resolution (it is
 * not a scope value); callers detect it before treating `base` as a literal
 * ref. A branch literally named `parent` can still be targeted explicitly
 * via `--base heads/parent`.
 */
...
const warnDiffUnavailable = (requested: RequestedScope, isQuiet: boolean): void => {
  ...
  // misleading — they told us exactly what to diff against. `parent` is
  // auto-detection, so a ref-name message would be equally misleading.
  if (requested.base === DIFF_PARENT_BASE) {
    logger.warn(
      "Could not detect a parent branch to diff against (no diverging branch found). Running full scan.",
    );
  } else if (typeof requested.base === "string") {
    logger.warn(
